...
import (
	...
	"github.com/hlandauf/btcnode/addrmgr"
...
// xAddKnownInventory adds the passed inventory to the cache of known inventory
// for the peer.  It is safe for concurrent access.
func (p *Peer) xAddKnownInventory(invVect *btcwire.InvVect) {
	p.knownInvMutex.Lock()
	defer p.knownInvMutex.Unlock()
...
// xVersionKnown returns the whether or not the version of a peer is known locally.
// It is safe for concurrent access.
func (p *Peer) xVersionKnown() bool {
	p.statsMtx.Lock()
...
func (p *Peer) xProtocolVersion() uint32 {
	p.statsMtx.Lock()
	defer p.statsMtx.Unlock()
...
// It is safe for concurrent access.
func (p *Peer) xRelayTxDisabled() bool {
	p.relayMtx.Lock()
...
func (p *Peer) pushVersionMsg() error {
	_, blockNum, err := p.node.db.NewestSha()
	if err != nil {
		return err
	}
	...
	if p.node.cfg.Proxy != "" {
		proxyaddress, _, err := net.SplitHostPort(p.node.cfg.Proxy)
	...
	msg := btcwire.NewMsgVersion(
		p.node.addrManager.GetBestLocalAddress(p.na), theirNa,
		p.node.nonce, int32(blockNum))
...
func (p *Peer) updateAddresses(msg *btcwire.MsgVersion) {
	...
	if !p.inbound {
		...
		if !p.node.cfg.DisableListen /* && isCurrent? */ {
			// Get address that best matches.
			lna := p.node.addrManager.GetBestLocalAddress(p.na)
		...
		hasTimestamp := p.xProtocolVersion() >=
			btcwire.NetAddressTimeVersion
		if p.node.addrManager.NeedMoreAddresses() && hasTimestamp {
			p.xQueueMessage(btcwire.NewMsgGetAddr(), nil)
		}

		// Mark the address as a known good address.
		p.node.addrManager.Good(p.na)
	} else {
		...
		if addrmgr.NetAddressKey(&msg.AddrMe) == addrmgr.NetAddressKey(p.na) {
			p.node.addrManager.AddAddress(p.na, p.na)
			p.node.addrManager.Good(p.na)
		}
	}
}
...
func (p *Peer) handleVersionMsg(msg *btcwire.MsgVersion) {
	...
	if msg.ProtocolVersion < int32(btcwire.MultipleAddressVersion) {
		...
		p.xPushRejectMsg(msg.Command(), btcwire.RejectObsolete, reason,
			nil, true)
		p.xDisconnect()
		return
	}
	...
	if p.versionKnown {
		...
		p.xPushRejectMsg(msg.Command(), btcwire.RejectDuplicate,
	...
	// Send verack.
	p.xQueueMessage(btcwire.NewMsgVerAck(), nil)
	...
	// discovered peers.
	if !p.node.cfg.SimNet {
		p.updateAddresses(msg)
	}

	// Add the remote peer time as a sample for creating an offset against
	// the local clock to keep the network time in sync.
	p.node.TimeSource.AddTimeSample(p.addr, msg.Timestamp)

	// Signal the block manager this peer is a new sync candidate.
	p.node.blockManager.NewPeer(p)
...
func (p *Peer) pushTxMsg(sha *btcwire.ShaHash, doneChan, waitChan chan struct{}) error {
	...
	tx, err := p.node.TxMemPool.FetchTransaction(sha)
	if err != nil {
	...
	p.xQueueMessage(tx.MsgTx(), doneChan)
...
func (p *Peer) pushBlockMsg(sha *btcwire.ShaHash, doneChan, waitChan chan struct{}) error {
	blk, err := p.node.db.FetchBlockBySha(sha)
	if err != nil {
		log.Tracef("Unable to fetch requested block sha %v: %v",
			sha, err)
	...
	p.xQueueMessage(blk.MsgBlock(), dc)
...
func (p *Peer) pushMerkleBlockMsg(sha *btcwire.ShaHash, doneChan, waitChan chan struct{}) error {
	...
	txList := p.node.db.FetchTxByShaList(matchedHashes)
...
func (p *Peer) xPushGetBlocksMsg(locator btcchain.BlockLocator, stopHash *btcwire.ShaHash) error {
...
// and stop hash.  It will ignore back-to-back duplicate requests.
func (p *Peer) xPushGetHeadersMsg(locator btcchain.BlockLocator, stopHash *btcwire.ShaHash) error {
	// XXX
	if p.protocolVersion < 70000 {
		return p.xPushGetBlocksMsg(locator, stopHash)
	}
...
func (p *Peer) xPushRejectMsg(command string, code btcwire.RejectCode, reason string, hash *btcwire.ShaHash, wait bool) {
	// Don't bother sending the reject message if the protocol version
	// is too low.
	if p.xVersionKnown() && p.xProtocolVersion() < btcwire.RejectVersion {
		return
	}
...
func (p *Peer) handleMemPoolMsg(msg *btcwire.MsgMemPool) {
	...
	txDescs := p.node.TxMemPool.TxDescs()
	...
	for i, txDesc := range txDescs {
		...
		hash := txDesc.Tx.Sha()
		if !p.node.TxMemPool.xIsTransactionInPool(hash) {
			continue
		}
...
func (p *Peer) handleTxMsg(msg *btcwire.MsgTx) {
	...
	p.node.blockManager.QueueTx(tx, p)
	<-p.txProcessed
}
...
func (p *Peer) handleBlockMsg(msg *btcwire.MsgBlock, buf []byte) {
	...
	p.node.blockManager.QueueBlock(block, p)
	<-p.blockProcessed
}
...
func (p *Peer) handleInvMsg(msg *btcwire.MsgInv) {
	p.node.blockManager.QueueInv(msg, p)
}
...
func (p *Peer) handleHeadersMsg(msg *btcwire.MsgHeaders) {
	p.node.blockManager.QueueHeaders(msg, p)
}
...
func (p *Peer) handleGetDataMsg(msg *btcwire.MsgGetData) {
	...
	if len(notFound.InvList) != 0 {
		p.xQueueMessage(notFound, doneChan)
	}
...
func (p *Peer) handleGetBlocksMsg(msg *btcwire.MsgGetBlocks) {
	...
	if !msg.HashStop.IsEqual(&zeroHash) {
		height, err := p.node.db.FetchBlockHeightBySha(&msg.HashStop)
		if err == nil {
			endIdx = height + 1
		}
	}
	...
	for _, hash := range msg.BlockLocatorHashes {
		height, err := p.node.db.FetchBlockHeightBySha(hash)
		if err == nil {
	...
	for start := startIdx; start < endIdx; {
		...
		hashList, err := p.node.db.FetchHeightRange(start, endIdx)
		if err != nil {
			log.Warnf("Block lookup failed: %v", err)
			return
		}
...
func (p *Peer) handleGetHeadersMsg(msg *btcwire.MsgGetHeaders) {
	...
	if len(msg.BlockLocatorHashes) == 0 {
		...
		header, err := p.node.db.FetchBlockHeaderBySha(&msg.HashStop)
		if err != nil {
			log.Warnf("Lookup of known block hash failed: %v",
				err)
			return
		}
		...
		p.xQueueMessage(headersMsg, nil)
		return
	}
	...
	for start := startIdx; start < endIdx; {
		...
		hashList, err := p.node.db.FetchHeightRange(start, endIdx)
		if err != nil {
			log.Warnf("Header lookup failed: %v", err)
			return
		}
		...
		for _, hash := range hashList {
			header, err := p.node.db.FetchBlockHeaderBySha(&hash)
			if err != nil {
				log.Warnf("Lookup of known block hash "+
					"failed: %v", err)
				continue
			}
			headersMsg.AddBlockHeader(header)
...
func (p *Peer) handleGetAddrMsg(msg *btcwire.MsgGetAddr) {
	...
	addrCache := p.node.addrManager.AddressCache()
...
func (p *Peer) handleAddrMsg(msg *btcwire.MsgAddr) {
	...
	if p.xProtocolVersion() < btcwire.NetAddressTimeVersion {
		return
	}
	...
	// same?
	p.node.addrManager.AddAddresses(msg.AddrList, p.na)
}
...
func (p *Peer) handlePingMsg(msg *btcwire.MsgPing) {
	...
	if p.xProtocolVersion() > btcwire.BIP0031Version {
		// Include nonce from ping so pong can be identified.
		p.xQueueMessage(btcwire.NewMsgPong(msg.Nonce), nil)
	}
}
...
func (p *Peer) readMessage() (btcwire.Message, []byte, error) {
	n, msg, buf, err := btcwire.ReadMessageN(p.conn, p.xProtocolVersion(),
		p.btcnet)
	p.statsMtx.Lock()
	p.bytesReceived += uint64(n)
	p.statsMtx.Unlock()
	p.node.addBytesReceived(uint64(n))
	if err != nil {
		return nil, nil, err
	}
	...
	log.Debugf("%v", xlog.LogClosure(func() string {
...
func (p *Peer) writeMessage(msg btcwire.Message) {
	...
	log.Tracef("%v", xlog.LogClosure(func() string {
		...
		err := btcwire.WriteMessage(&buf, msg, p.xProtocolVersion(),
			p.btcnet)
		if err != nil {
			return err.Error()
		}
	...
	n, err := btcwire.WriteMessageN(p.conn, msg, p.xProtocolVersion(),
		p.btcnet)
	p.statsMtx.Lock()
	p.bytesSent += uint64(n)
	p.statsMtx.Unlock()
	p.node.addBytesSent(uint64(n))
	if err != nil {
		p.xDisconnect()
...
out:
	for atomic.LoadInt32(&p.disconnect) == 0 {
		...
		if err != nil {
			...
			// allowed errors.
			if p.node.cfg.RegressionTest && p.isAllowedByRegression(err) {
		...
		if vmsg, ok := rmsg.(*btcwire.MsgVersion); !ok && !p.xVersionKnown() {
			errStr := "A version message must precede all others"
			p.logError(errStr)

			// Push a reject message and wait for the message to be
			// sent before disconnecting.
			p.xPushRejectMsg(vmsg.Command(), btcwire.RejectMalformed,
				errStr, nil, true)
			break out
		}
		...
		default:
			log.Debugf("Received unhandled message of type %v: Fix Me",
				rmsg.Command())
		}
		...
		if markConnected && atomic.LoadInt32(&p.disconnect) == 0 {
			...
			p.node.addrManager.Connected(p.na)
		}
		// ok we got a message, reset the timer.
		// timer just calls p.xDisconnect() after logging.
		idleTimer.Reset(idleTimeoutMinutes * time.Minute)
		p.retryCount = 0
	}
	...
	if p.xVersionKnown() {
		p.node.blockManager.DonePeer(p)
	}
...
func (p *Peer) outHandler() {
	pingTimer := time.AfterFunc(pingTimeoutMinutes*time.Minute, func() {
		...
		p.xQueueMessage(btcwire.NewMsgPing(nonce), nil)
	})
out:
	for {
		select {
...
func (p *Peer) xQueueMessage(msg btcwire.Message, doneChan chan struct{}) {
...
// concurrent access.
func (p *Peer) xQueueInventory(invVect *btcwire.InvVect) {
...
func newPeerBase(n *Node, inbound bool) *Peer {
	p := Peer{
		node:            n,
		protocolVersion: MaxProtocolVersion,
		btcnet:          n.cfg.ActiveNetParams.Net,
...
func newInboundPeer(n *Node, conn net.Conn) *Peer {
	p := newPeerBase(n, true)
	p.conn = conn
...
func newOutboundPeer(n *Node, addr string, persistent bool, retryCount int64) *Peer {
	p := newPeerBase(n, false)
	p.addr = addr
	...
	p.na, err = n.addrManager.HostToNetAddress(host, uint16(port), 0)
	if err != nil {
	...
	go func() {
		...
		if p.retryCount > 0 {
			...
			log.Debugf("Retrying connection to %s in %s", addr, scaledDuration)
			time.Sleep(scaledDuration)
		}
		log.Debugf("Attempting to connect to %s", addr)
		conn, err := p.node.btcdDial("tcp", addr)
		if err != nil {
		...
		if atomic.LoadInt32(&p.disconnect) == 0 {
			p.timeConnected = time.Now()
			p.node.addrManager.Attempt(p.na)
