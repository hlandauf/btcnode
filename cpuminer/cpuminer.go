...
var log, Log = xlog.New("btc.node.cpuminer")
...
func (m *CPUMiner) submitBlock(block *btcutil.Block) bool {
	...
	latestHash, _ := m.node.ChainState().Best()
	...
	isOrphan, err := m.node.ProcessBlock(block, btcchain.BFNone)
	if err != nil {
...
func (m *CPUMiner) solveBlock(msgBlock *btcwire.MsgBlock, blockHeight int64,
	...
	lastTxUpdate := m.node.TxMemPool.LastUpdated()
	...
	for extraNonce := uint64(0); extraNonce < maxExtraNonce; extraNonce++ {
		...
		btcnode.UpdateExtraNonce(msgBlock, blockHeight, extraNonce+enOffset)
		...
		for i := uint32(0); i <= maxNonce; i++ {
			...
			case <-ticker.C:
				...
				// has changed.
				bestHash, _ := m.node.ChainState().Best()
				...
				// minute.
				if lastTxUpdate != m.node.TxMemPool.LastUpdated() &&
					time.Now().After(lastGenerated.Add(time.Minute)) {

					return false
				}

				m.node.UpdateBlockTime(msgBlock)
...
out:
	for {
		...
		m.submitBlockLock.Lock()
		_, curHeight := m.node.ChainState().Best()
		if curHeight != 0 && !m.node.IsCurrent() {
		...
		payToAddr := m.node.Config().MiningAddrsS[rand.Intn(len(m.node.Config().MiningAddrsS))]

		// Create a new block template using the available transactions
		// in the memory pool as a source of transactions to potentially
		// include in the block.
		template, err := btcnode.NewBlockTemplate(m.node.TxMemPool, payToAddr)
		m.submitBlockLock.Unlock()
		if err != nil {
...
func New(n *btcnode.Node) *CPUMiner {
	return &CPUMiner{
		node:              n,
