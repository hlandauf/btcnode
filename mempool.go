...
type notifyMempoolTxFunc func(tx *btcutil.Tx, isNew bool, lastUpdated time.Time)
...
type TxMemPool struct {
	...
	NotifyMempoolTxFunc notifyMempoolTxFunc
}
...
func checkTransactionStandard(tx *btcutil.Tx, height int64, netParams *btcnet.Params) error {
	msgTx := tx.MsgTx()

	// The transaction must be a currently supported version.
	if !(netParams.AllowNamecoin && msgTx.Version == namecoinTxVersion) &&
		(msgTx.Version > btcwire.TxVersion || msgTx.Version < 1) {
...
func checkInputsStandard(tx *btcutil.Tx, txStore btcchain.TxStore, netParams *btcnet.Params) error {
...
func (mp *TxMemPool) xRemoveOrphan(txHash *btcwire.ShaHash) {
	mp.Lock()
...
func (mp *TxMemPool) addOrphan(tx *btcutil.Tx) {
	...
	log.Debugf("Stored orphan transaction %v (total: %d)", tx.Sha(),
		len(mp.orphans))
}
...
//
// This function is safe for concurrent access.
func (mp *TxMemPool) xIsTransactionInPool(hash *btcwire.ShaHash) bool {
...
//
// This function is safe for concurrent access.
func (mp *TxMemPool) xIsOrphanInPool(hash *btcwire.ShaHash) bool {
	// Protect concurrent access.
	mp.RLock()
	defer mp.RUnlock()
...
//
// This function is safe for concurrent access.
func (mp *TxMemPool) xRemoveTransaction(tx *btcutil.Tx) {
	// Protect concurrent access.
	mp.Lock()
	defer mp.Unlock()
...
//
// This function is safe for concurrent access.
func (mp *TxMemPool) xRemoveDoubleSpends(tx *btcutil.Tx) {
	// Protect concurrent access.
	mp.Lock()
	defer mp.Unlock()
...
func (mp *TxMemPool) fetchInputTransactions(tx *btcutil.Tx) (btcchain.TxStore, error) {
	txStore, err := mp.node.blockManager.blockChain.FetchTransactionStore(tx)
	if err != nil {
		return nil, err
	}
...
func (mp *TxMemPool) maybeAcceptTransaction(tx *btcutil.Tx, isOrphan *bool, isNew, rateLimit bool) error {
	...
	_, curHeight, err := mp.node.db.NewestSha()
	if err != nil {
	...
	if !mp.node.cfg.ActiveNetParams.RelayNonStdTxs {
		err := checkTransactionStandard(tx, nextBlockHeight, mp.node.cfg.ActiveNetParams)
		if err != nil {
			// Attempt to extract a reject code from the error so
			// it can be retained.  When not possible, fall back to
			// a non standard error.
	...
	// used later.
	txFee, err := btcchain.CheckTransactionInputs(tx, nextBlockHeight, txStore, mp.node.db.NameDB())
	if err != nil {
		if cerr, ok := err.(btcchain.RuleError); ok {
			return chainRuleError(cerr)
		}
	...
	if !mp.node.cfg.ActiveNetParams.RelayNonStdTxs {
		err := checkInputsStandard(tx, txStore, mp.node.cfg.ActiveNetParams)
		if err != nil {
			// Attempt to extract a reject code from the error so
			// it can be retained.  When not possible, fall back to
			// a non standard error.
	...
	if rateLimit && txFee < minFee {
		...
		if mp.pennyTotal >= mp.node.cfg.FreeTxRelayLimit*10*1000 {
		...
		log.Tracef("rate limit: curTotal %v, nextTotal: %v, "+
			...
			mp.node.cfg.FreeTxRelayLimit*10*1000)
	}
	...
	if mp.NotifyMempoolTxFunc != nil {
		mp.NotifyMempoolTxFunc(tx, isNew, mp.lastUpdated)
	}

	/*if mp.node.rpcServer != nil {
		// Notify websocket clients about mempool transactions.
		mp.node.rpcServer.ntfnMgr.NotifyMempoolTx(tx, isNew)

		// Potentially notify any getblocktemplate long poll clients
		// about stale block templates due to the new transaction.
		mp.node.rpcServer.gbtWorkState.NotifyMempoolTx(mp.lastUpdated)
	}*/
...
//
// This function is safe for concurrent access.
func (mp *TxMemPool) xMaybeAcceptTransaction(tx *btcutil.Tx, isOrphan *bool, isNew, rateLimit bool) error {
	// Protect concurrent access.
	mp.Lock()
	defer mp.Unlock()
...
func (mp *TxMemPool) ProcessTransaction(tx *btcutil.Tx, allowOrphan, rateLimit bool) error {
	...
	log.Tracef("Processing transaction %v", tx.Sha())
...
//
// This function is safe for concurrent access.
func (mp *TxMemPool) xTxShas() []*btcwire.ShaHash {
	mp.RLock()
	defer mp.RUnlock()
...
func newTxMemPool(node *Node) *TxMemPool {
	return &TxMemPool{
		node:          node,
