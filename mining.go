...
func createCoinbaseTx(coinbaseScript []byte, nextBlockHeight int64, addr btcutil.Address, params *btcnet.Params) (*btcutil.Tx, error) {
...
func logSkippedDeps(mempool *TxMemPool, tx *btcutil.Tx, deps *list.List) {
	if deps == nil {
		return
	}
...
// rules.
func (c *ChainState) MinimumMedianTime() (time.Time, error) {
	c.m.Lock()
	defer c.m.Unlock()
	if c.PastMedianTimeErr != nil {
		return time.Time{}, c.PastMedianTimeErr
	}

	return c.PastMedianTime.Add(time.Second), nil
}
...
func NewBlockTemplate(mempool *TxMemPool, payToAddress btcutil.Address) (*BlockTemplate, error) {
	blockManager := mempool.node.blockManager
	...
	coinbaseTx, err := createCoinbaseTx(coinbaseScript, nextBlockHeight,
		payToAddress, mempool.node.cfg.ActiveNetParams)
	if err != nil {
		return nil, err
	}
	...
	sortedByFee := mempool.node.cfg.BlockPrioritySize == 0
	...
	log.Debugf("Considering %d mempool transactions for inclusion to "+
...
mempoolLoop:
	for _, txDesc := range mempoolTxns {
		...
		if btcchain.IsCoinBase(tx) {
			log.Tracef("Skipping coinbase tx %s", tx.Sha())
			continue
		}
		if !btcchain.IsFinalizedTransaction(tx, nextBlockHeight, time.Now()) {
			log.Tracef("Skipping non-finalized tx %s", tx.Sha())
			continue
		}
	...
	for priorityQueue.Len() > 0 {
		...
		if blockPlusTxSize < blockSize || blockPlusTxSize >= mempool.node.cfg.BlockMaxSize {
			log.Tracef("Skipping tx %s because it would exceed "+
				"the max block size", tx.Sha())
			logSkippedDeps(mempool, tx, deps)
			continue
		}
		...
		if sortedByFee && prioItem.feePerKB < MinTxRelayFee &&
			blockPlusTxSize >= mempool.node.cfg.BlockMinSize {
		...
		// transactions.
		if !sortedByFee && (blockPlusTxSize >= mempool.node.cfg.BlockPrioritySize ||
			prioItem.priority <= minHighPriority) {

			log.Tracef("Switching to sort by fees per "+
				"kilobyte blockSize %d >= BlockPrioritySize "+
				"%d || priority %.2f <= minHighPriority %.2f",
				blockPlusTxSize, mempool.node.cfg.BlockPrioritySize,
				prioItem.priority, minHighPriority)
			...
			if blockPlusTxSize > mempool.node.cfg.BlockPrioritySize ||
				prioItem.priority < minHighPriority {
		...
		_, err = btcchain.CheckTransactionInputs(tx, nextBlockHeight,
			blockTxStore, mempool.node.db.NameDB())
		if err != nil {
			log.Tracef("Skipping tx %s due to error in "+
...
func updateBlockTime(msgBlock *btcwire.MsgBlock, bManager *blockManager) error {
	...
	// do so now.
	if bManager.node.cfg.ActiveNetParams.ResetMinDifficulty {
...
func (n *Node) UpdateBlockTime(msgBlock *btcwire.MsgBlock) error {
	return updateBlockTime(msgBlock, n.blockManager)
}
